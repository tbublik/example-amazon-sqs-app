...
class Analyzer:
    def job(self):
        ...
        sqsUrl = boto3.client("ssm").get_parameter(Name="sqsUrl", WithDecryption=False)[
            "Parameter"
        ]["Value"]
        ...
        for message in messages:
            ...
            s3_response = s3.Object(bucket_name=s3bucket, key=s3key)
            # Transform image body to array of bytes to feed to cv2.imdecode
            file_bytes = numpy.frombuffer(
                s3_response.get()["Body"].read(), dtype=numpy.uint8
            )
            ...
            logger.info(
                f"""{item} has been saved in DynamoDB {imageAnalyticsTableName} table."""
            )
...
if __name__ == "__main__":
    while 1:
        try:
            logger.info(
                f"""Job started [{strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())}]"""
            )
            Analyzer().job()
            logger.info(
                f"""Job finished [{strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())}]"""
            )
            time.sleep(10)
        except:
            raise
